...
constexpr double channel::normalize(color_t color) noexcept {
  return static_cast<double>(color) / MAX_COLOR;
}

constexpr color_t channel::denormalize(
    std::floating_point auto normed) noexcept {
  assert(0. <= normed && normed <= 1.);
  const auto expanded = std::lround(MAX_COLOR * normed);
  return static_cast<color_t>(expanded);
}

constexpr bool channel::almost_eq(std::floating_point auto lhs,
                                  std::floating_point auto rhs) noexcept {
